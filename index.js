...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Adeus @${num.split('@')[0]} Já foi tarde👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["994406132694@s.whatsapp.net"] // replace this with your number
			...
			switch(command) {
				...
				case 'gtts':
					...
					ranm = getRandom('.mp3')
					dtt.length > 600
					? reply('O texto é muito grande tio')
...
case 'boa noite':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://cataas.com/cat/says/Boa%20Noite`)
...
case 'gato fofo':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://cataas.com/cat/cute`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Gatinho fofo...'})
					break
case 'a mimir':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://cataas.com/cat/sleep/says/a%20mimir`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'A mimir...'})
					break
				/*case 'memeindo':
					memein = await kagApi.memeindo()
				...
				case 'nsfwloli':
					...
					loli.getNSFWLoli(async (err, res) => {
						if (err) return reply('❌ *ERRO* ❌')
						buffer = await getBuffer(res.url)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Safado!'})
					})
					break*/
				case 'hilih':
				...
				case 'tiktokstalk':
					try {
						if (args.length < 1) return client.sendMessage(from, 'Cade o username, hum?', text, {quoted: mek})
				...
				case 'tulis':
					...
					client.sendMessage(from, buff, image, {quoted: mek, caption: mess.sucess})
					break
				case 'url2img':
				...
				case 'bc':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Promova o sucesso @${mentioned[0].split('@')[0]} Agora é um adiministrador do grupo!`, mentioned, true)
				...
				case 'add':
					...
					if (args[0].startsWith('08')) return reply('Use o código do país válido')
					try {
				...
				case 'listadmins':
					...
					break
                                ...
                                case 'leave':
                                        ...
                                        break
				case 'toimg':
					if (!isQuotedSticker) return reply('❌ Erro, marque uma figurinha ❌')
					reply(mess.wait)
